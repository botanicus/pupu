...
Pupu.media_directory = File.join(MyFramework.root, "media") # you don't have to set it if you are using public or media
...
Initializers are the only part of pupu which are not stored in <code>root/pupu</code>, but are copyied to <code>[media_directory]/javascripts/initializers</code> resp. <code>[media_directory]/stylesheets/initializers</code>. In contrary to files in <code>[media_directory]/pupu</code>, initializers are intended for user editing.
