...
Merb_pupu is a plugin system for merb's public stuff like mootools plugins, icon sets etc. It knows dependencies and it has CLI interface, so it's really easy to bundle such pupus into your app.

h1. Why I wrote it

In my apps the public/ stuff always get bloated and messy when I have all javascripts in public/javascripts, all stylesheets in public/stylesheets etc. It's nearly imposible to remove some stuff related with let's say some mootools plugin. It has it's own javascripts, stylesheets, few images + code which is required for initialization. And of course it is really bad idea to put your own code to directory with such things as javascript framework is.
...
Everything what can be putted into public directory. Some examples: blueprint css, mootools, autocompleter for mootools, icon set ...
...
pupu :autocompleter, :request => "local"
</pre>
...
Initializers are the only part of merb_pupu which are not stored in <code>public/pupu</code>, but are copyied to <code>public/javascripts/initializers</code> and <code>public/stylesheets/initializers</code>. In contrary of files in <code>public/pupu</code>, initializers are intended for user editing.
...
* MooTools <code>pupu :mootools, :more => true</code>
* Autocompleter
* Squeezebox
* Blueprint CSS
* Mooeditable
...
<pre>
merb-gen pupu chunky_bacon helpers.js blue.css
</pre>
